...
description: |
  ...
  "deepsearch", "deep search", "deep-search", "research this topic", "write a report on", "survey the literature on", "competitive analysis of",
...
In the local Gemma Codex runtime, use Context7 for this skill's research steps. Treat `web_search`
and `web_fetch` references below as the local Context7 workflow: call `resolve-library-id`, then
`query-docs`, or use `gemma_agent context7_search` when available. Do not stop because generic
web search tools are unavailable if Context7 is available.

Alias routing: `deepsearch`, `deep search`, and `deep-search` mean this `deep-research` skill.
Use Context7 as the research source inside this workflow, not as the replacement skill.
